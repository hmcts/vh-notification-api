...
[ApiController]
[Route("")]
public class ParticipantEmailNotificationsController(
    ...
    : ControllerBase
{
        ...
        string parametersJson, Dictionary<string, string> parameters)
    {
        ...
        await createNotificationService.CreateEmailNotificationAsync(notification, parameters);
    }
    
    private async Task AreAllParamsGiven(Dictionary<string, string> parameters, NotificationType notificationType)
    {
        var template =
            await queryHandler.Handle<GetTemplateByNotificationTypeQuery, Template>(
